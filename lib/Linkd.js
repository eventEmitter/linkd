!function() {
	...
	var classDefinition = {
		...
		, init: function(CustomType) {
			...
			if (CustomType){
				if (!(CustomType instanceof Node)) throw new Error('The custom type must inherit from the LinkedList.Node class!');
		...
		, _removeNode: function(hash) {
			...
			else throw new Error('Cannot remove the node «'+hash+'», the node doesn\'t exist!');
		}
	};
